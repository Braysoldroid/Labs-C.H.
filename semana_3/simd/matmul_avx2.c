...
{
    //Multiplicar los dos vectores: Ejercicio A

    return _mm256_mul_ps(a, b); 
    //mm256_mul_ps realiza una multiplicacion de dos
    //elementos de 32 bits c/u (single precision).
}

static inline float simd_reduce_add_ps(__m256 value)
{
    //Reducir los vectores: Ejercicio B

    __m128 low = _mm256_extractf128_ps(value, 0); //Extrae la parte baja
    __m128 high = _mm256_extractf128_ps(value, 1); //Extrae la parte alta

    __m128 sum = _mm_add_ps(low, high); //Parte baja + parte alta
...
{
    //Producto punto para multiplciacion de matrices
    float result = 0.0f;

    //For loop que brinque de AVX_FLOATS elementos
    ...
    for (int i = 0; i < VECTOR_SIZE; i += AVX_FLOATS){
        __m256 a_vec = simd_loadu_ps(&a[i]); //Puntero que apunta al "i"vo elemento
